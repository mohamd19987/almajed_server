...
# -----------------------------
# البحث المتقدم (للبحث بالاسم + الهاتف + التاريخ)
# -----------------------------
...
def get_invoice(invoice_id):
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM bookings WHERE id=?", (invoice_id,))
    row = cursor.fetchone()
    conn.close()

    if not row:
        return jsonify({"status": "error", "msg": "الفاتورة غير موجودة"}), 404

    return jsonify({"status": "ok", "invoice": dict(row)})
